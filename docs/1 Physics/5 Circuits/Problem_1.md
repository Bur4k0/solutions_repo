...
Function CalculateEquivalentResistance(Graph G, Node source, Node sink):
    Input: Graph G (nodes, edges with resistance weights), source node, sink node
    Output: Equivalent resistance between source and sink

    While G has more than 2 nodes OR more than 1 edge between source and sink:
        // Step 1: Check for series reductions
        For each node N in G:
            If degree(N) == 2:
                Let N1 and N2 be the neighbors of N
                Let R1 = resistance of edge (N1, N)
                Let R2 = resistance of edge (N, N2)
                If N1 != N2:  // Avoid self-loops
                    R_series = R1 + R2
                    Remove node N and edges (N1, N), (N, N2)
                    Add edge (N1, N2) with weight R_series

        // Step 2: Check for parallel reductions
        For each pair of nodes (A, B) in G:
            If multiple edges exist between A and B:
                Let R_values = list of resistances between A and B
                R_parallel = 1 / (sum(1/R for R in R_values))
                Remove all edges between A and B
                Add edge (A, B) with weight R_parallel

    // Step 3: Return result
    If G has exactly 2 nodes (source, sink) and 1 edge:
        Return resistance of edge (source, sink)
    Else:
